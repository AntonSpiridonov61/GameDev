...
function Spaceship:create(position)
    ...
    spaceship.maxVerticalSpeed = 0.2
...
function Spaceship:draw()
    ...
    if self.active then
        if os.clock() - self.activeDuration > 0.5 then
            love.graphics.polygon('line', { -3, 5, 3, 5, 0, 15 + love.math.random(10) })
        else
            love.graphics.polygon('line', { -3, 5, 3, 5, 0, 15 + love.math.random(self.activeDuration / 5) })
        end
    else
        if os.clock() - self.inactiveDuration < 0.1 then
            love.graphics.polygon('line', self.engineShed.big)
        elseif os.clock() - self.inactiveDuration < 0.2 then
            love.graphics.polygon('line', self.engineShed.middle)
        elseif os.clock() - self.inactiveDuration < 0.3 then
            love.graphics.polygon('line', self.engineShed.small)
        end
    end
